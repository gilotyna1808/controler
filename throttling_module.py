...
class throttling_check:
    ...
    def get_if_current_under_voltage(self):
        if int(self.current_under_voltage) == 1: return 1
        else: return 0

    def get_if_current_throttling(self):
        if int(self.current_throttling == 1): return 1
        else: return 0

    def get_if_occurred_under_voltage(self):
        if int(self.occurred_under_voltage) == 1: return 1
        else: return 0

    def get_if_occurred_throttling(self):
        if int(self.occurred_throttling) == 1: return 1
        else: return 0
